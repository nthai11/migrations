-- Temporary compatibility for the TMS-enabled develop stack. Migration 1001
-- moved authentication integrations to the new integration model, but the
-- currently checked-out service-authorization develop branch still reads the
-- legacy integration types and oauth_registration tables. Keep both schemas
-- available until service-authorization adopts the new model. Once published,
-- keep this migration immutable and add a new forward migration to remove the
-- compatibility objects. See README.md for the removal checklist.
